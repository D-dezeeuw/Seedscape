...
import { createWorldNoiseV2, generateChunkV2, sampleHeight, sampleMoisture } from "./generation_v2";

describe("generation_v2 determinism", () => {
  test("same seed + chunk → identical bytes", () => {
    const a = createWorldNoiseV2(42);
    const b = createWorldNoiseV2(42);
    const c1 = generateChunkV2(a, 0, 0);
    const c2 = generateChunkV2(b, 0, 0);
    expect(Array.from(c1.tileId)).toEqual(Array.from(c2.tileId));
  });

  test("different seeds produce different chunks at the same coords", () => {
    const c1 = generateChunkV2(createWorldNoiseV2(1), 0, 0);
    const c2 = generateChunkV2(createWorldNoiseV2(2), 0, 0);
  ...
  test("different chunks at the same seed have different tile patterns", () => {
    const noise = createWorldNoiseV2(7);
    const a = generateChunkV2(noise, 0, 0);
    const b = generateChunkV2(noise, 1, 0);
    expect(Array.from(a.tileId)).not.toEqual(Array.from(b.tileId));
  });
});

describe("generation_v2 field samplers", () => {
  test("sampleHeight is reproducible per (seed, x, y)", () => {
    const a = sampleHeight(createWorldNoiseV2(99), 12.5, -3);
    const b = sampleHeight(createWorldNoiseV2(99), 12.5, -3);
    expect(a).toBe(b);
  });

  test("sampleHeight stays roughly in [-1, 1]", () => {
    const noise = createWorldNoiseV2(99);
    let max = -Infinity;
    let min = Infinity;
    for (let y = 0; y < 64; y++) {
      for (let x = 0; x < 64; x++) {
...
describe("generation_v2 produces a healthy mix of tiles", () => {
  test("a single chunk somewhere along a varied gradient surfaces multiple tile ids", () => {
    ...
    const noise = createWorldNoiseV2(0xc0ffee);
    const tiles = new Set<number>();
    for (let cx = 0; cx < 3; cx++) {
      for (let cy = 0; cy < 3; cy++) {
        const c = generateChunkV2(noise, cx, cy);
...
describe("generation_v2 reuses preallocated buffers", () => {
  test("writing to `out` mutates the same arrays without allocating", () => {
    const noise = createWorldNoiseV2(1);
    const out = generateChunkV2(noise, 0, 0);
    const buf = out.tileId.buffer;
    generateChunkV2(noise, 1, 0, out);
    ...
    const fresh = generateChunkV2(noise, 0, 0);
