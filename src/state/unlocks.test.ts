...
describe("unlocksForLevel", () => {
  test("level 1 unlocks wheat + carrot seeds (starter set)", () => {
  ...
  test("level 5 has wheat, carrot, mill, bakery; corn still locked", () => {
...
describe("newUnlocksAtLevel", () => {
  test("returns only entries whose required level matches", () => {
    ...
    expect(ids).toEqual(["building.bakery"]);
  });
  ...
  test("level 4 unlocks Well + Chicken Pen (Phase 9)", () => {
    const ids = newUnlocksAtLevel(4)
      .map((u) => u.id)
      .sort();
    expect(ids).toEqual(["building.chicken_pen", "building.well"]);
  });

  test("level 6 unlocks Sprinkler + Cow Pen (Phase 9)", () => {
    const ids = newUnlocksAtLevel(6)
      .map((u) => u.id)
      .sort();
    expect(ids).toEqual(["building.cow_pen", "building.sprinkler"]);
  });

  test("empty when nothing unlocks at that level", () => {
    expect(newUnlocksAtLevel(8)).toEqual([]);
  });
});
...
describe("isBuildingUnlocked / isSeedUnlocked", () => {
  ...
  test("carrot seed (id 608) is unlocked from level 1 (starter inventory)", () => {
    expect(isSeedUnlocked(1, 608)).toBe(true);
  });
